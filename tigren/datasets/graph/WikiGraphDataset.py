...
from tigren.datasets.graph.GraphDataset import GraphDataset

def get_market_npy_url(date, market):
    return f"tigren/datasets/data/wiki/{date}/{market.upper()}_wiki_relation.npy"
def get_ticker_csv_url(date, market):
    return f"tigren/datasets/data/wiki/{date}/{market.upper()}_tickers.csv"
...
class WikiGraphDataset(GraphDataset):
    ...
    def load_local_wiki(self, date: str, market: str) -> Tuple[np.ndarray, List[str]]:
        ...
        """
        if os.path.isdir(f"tigren/datasets/data/wiki/{date}"):
